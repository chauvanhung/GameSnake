...
namespace AI_Snake.Snake
{
    ...
    class Snake
    {
        ...
        {
            //vẽ đầu rắn
            Render.Draw(g, new Point(snakeBody[0].X * 10, snakeBody[0].Y * 10), fringe, headColor);
            //vẽ thân rắn
            for (int i = 1; i < length; i++)
                //g.FillRectangle(bodyColor, snakeBody[i].X * 10, snakeBody[i].Y * 10, 10, 10);
                Render.Draw(g, new Point(snakeBody[i].X * 10, snakeBody[i].Y * 10), fringe, bodyColor);
        }

        /// <summary>
        /// Cập nhật lại mảng vị trí của con rắn và vẽ lại con rắn luôn.
        /// </summary>
        public void Move(Graphics g)
        {
            //xóa đuôi của con rắn trên graphic
            Render.Draw(g, new Point(snakeBody[length - 1].X * 10, snakeBody[length - 1].Y * 10), Pens.White, Brushes.White);
        ...
        public void Clear(Graphics g)
        {
            for (int i = 0; i < length; i++)
                Render.Draw(g, new Point(snakeBody[i].X * 10, snakeBody[i].Y * 10), Pens.White, Brushes.White);
        }
